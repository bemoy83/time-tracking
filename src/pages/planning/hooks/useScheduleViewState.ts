...
import { useScheduleAssistantState } from './useScheduleAssistantState';
...
import { getSchedulableUnscheduledPhaseRowCount } from '../schedule/ScheduleGrid';
...
interface UseScheduleViewStateProps {
  plan: Plan;
  onSave: (plan: Plan) => void;
  readOnly: boolean;
  onScheduleContextChange?: (ctx: ScheduleEditContextValue | null) => void;
  scheduleGridRef: RefObject<HTMLDivElement>;
}
...
export function useScheduleViewState({
  ...
  const {
    allocations: crewPoolAllocations,
    dailyDeployments: crewPoolDailyDeployments,
    defaultCrewSize: systemDefaultCrewSize,
    taskSwitchingFactor: crewPoolTaskSwitchingFactor,
  } = useCrewPoolStore();
  ...
  }, [workCalendarPhaseSpans, currentPlan.workCalendar.length, mutatePlan]);
  ...
  const scheduleCoverage = useMemo(
    () => buildScheduleCoverageMetric(planTotalPersonHours(currentPlan), capacity.totalRequiredPersonHours),
  ...
  const conflictSuggestions = useMemo(() => generateConflictSuggestions(capacity), [capacity]);
  ...
  const handlePlanDateChange = useCallback(
    (field: 'eventStartDate' | 'eventEndDate', value: string) => {
  ...
  const handlePlanPhaseDateChange = useCallback(
    (field: PhaseDateField, value: string) => {
      clearAssistantReport();
  ...
  const handleDefaultCrewChange = useCallback(
    (value: string) => {
      clearAssistantReport();
  ...
  const handleDefaultEfficiencyChange = useCallback(
    (value: string) => {
      clearAssistantReport();
  ...
  const handleAutoSchedule = useCallback(() => {
    ...
    const tagSequence =
      sequenceTagIds.length > 0 ? { id: 'global' as const, tagIds: sequenceTagIds, updatedAt: '' } : undefined;
  ...
  const handleSelectAssistantIssue = useCallback(
    (issueKey: string) => {
  ...
  const { planningIssues, panelIssues } = useMemo(
    () =>
      buildScheduleViewIssues({
        capacity,
        conflictSuggestions,
  ...
  const contextValue = useMemo(
    ...
    [
      ...
      handleDefaultCrewChange,
      handleDefaultEfficiencyChange,
